...
{
    public partial class frmAlternateIdentifyingArea : Form
    {
        ...
        public frmAlternateIdentifyingArea()
        {
            InitializeComponent();
        }

        private void frmAlternateIdentifyingArea_Load(object sender, EventArgs e)
        {
        ...
        private bool AlternatePopulateListBox()
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                lbColumnB.Items.Add(pair.Key);
                lbColumnA.Items.Add(pair.Value);
                cmbAnswerA.Items.Add(pair.Key);
                cmbAnswerB.Items.Add(pair.Key);
                cmbAnswerC.Items.Add(pair.Key);
                cmbAnswerD.Items.Add(pair.Key);
        ...
        private void btnCheckAnswers_Click(object sender, EventArgs e)
        {
            ...
            var myKeyA = availableCallingNumbers.FirstOrDefault(x => x.Key == valueA).Value;
            var myKeyB = availableCallingNumbers.FirstOrDefault(x => x.Key == valueB).Value;
            var myKeyC = availableCallingNumbers.FirstOrDefault(x => x.Key == valueC).Value;
            var myKeyD = availableCallingNumbers.FirstOrDefault(x => x.Key == valueD).Value;
            ...
            if (myKeyA.Equals(A) && myKeyB.Equals(B) && myKeyC.Equals(C) && myKeyD.Equals(D))
            {
                ...
                frmAlternateEndgame frmIA = new frmAlternateEndgame();
                this.Hide();
                frmIA.ShowDialog();
                this.Close();
            }
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            AlternatePopulateListBox(); // method call
